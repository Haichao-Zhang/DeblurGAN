    ...
    def blur_image(self, save=False, show=False):
        ...
        if len(psf) > 1:
            for p in psf:
                ...
                blured[:, :, 0] = np.array(signal.fftconvolve(self.original[:, :, 0], tmp, 'same'))
                blured[:, :, 1] = np.array(signal.fftconvolve(self.original[:, :, 1], tmp, 'same'))
                blured[:, :, 2] = np.array(signal.fftconvolve(self.original[:, :, 2], tmp, 'same'))
